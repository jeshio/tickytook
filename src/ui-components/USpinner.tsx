...
interface IUSpinnerProps extends React.PropsWithChildren<{}> {
  loading: boolean;
}

const Spinner = styled.div`
  color: ${({ theme }: ICWithTheme) => theme.colors.blue};
  font-size: 20px;
  margin: 0 auto;
  width: 1em;
  height: 1em;
  ...
  position: relative;
  text-indent: -9999em;
  -webkit-animation: u-spinner 1.3s infinite linear;
  animation: u-spinner 1.3s infinite linear;
  -webkit-transform: translateZ(0);
  -ms-transform: translateZ(0);
  transform: translateZ(0);
  transform: scale(0.5);

  @-webkit-keyframes u-spinner {
    0%,
    100% {
...
const USpinner: React.FunctionComponent<IUSpinnerProps> = props => {
  if (props.loading) {
    return (
      <UBlock textAlign="center" my="6rem">
        <Spinner />
      </UBlock>
    );
  }
